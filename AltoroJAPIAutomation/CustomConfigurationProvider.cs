...
public class CustomConfigurationProvider
{
    ...
    static CustomConfigurationProvider()
    {
        var builder = new ConfigurationBuilder().
                             SetBasePath(GetPath()).AddJsonFile("AltoroJTestData.json");
    ...
    {
        var obj = JObject.Parse(File.ReadAllText($@"{GetPath()}AltoroJTestData.json"));
    ...
    {
        if (key == "AltoroJService-baseUrl")
        {
#if (TestEnv)
key = "AltoroJService-baseUrl";
#endif
        }
        ...
        if (configuration != null)
        {
            ...
            if (value == null)
            {
                throw new Exception($"Could not find the key {key} in AltoroJTestData.json file");
            }
        }
        return value;
    }
